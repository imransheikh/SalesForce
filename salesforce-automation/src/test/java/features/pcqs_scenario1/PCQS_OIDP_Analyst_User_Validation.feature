Feature: Validate all expected feature is present in Application Receipt and Personal detail page and also validate different search criteria in cutomer search page
  Verify all expected feature is present in Application Receipt and Personal detail page with OIDP Analyst user
  @PCQSE2EOIDPANALYST
  Scenario: Log in to Saleforce application
    ...
    When For PCQS scenario Search for required Internal User "OIDP Analyst"
    Then For PCQS scenario Logging in as Internal user and verifying "OIDP Analyst"
  @PCQSE2EOIDPANALYST
