Feature: Validate all expected feature is present in Application Receipt and Personal detail page and also validate different search criteria in cutomer search page
  Verify all expected feature is present in Application Receipt and Personal detail page with OoC Internal User
  @PCQSE2EOOCINTUSR
  Scenario: Log in to Saleforce application
    ...
    When For PCQS scenario Search for required Internal User "OoC Internal User"
    Then For PCQS scenario Logging in as Internal user and verifying "OoC Internal User"
  @PCQSE2EOOCINTUSR
