Feature: Validate all expected feature is present in Application Receipt and Personal detail page and also validate different search criteria in cutomer search page
  Verify all expected feature is present in Application Receipt and Personal detail page with OCC Attorney user
  @PCQSE2EOCCATT
  Scenario: Log in to Saleforce application
    ...
    When For PCQS scenario Search for required Internal User "OCC Attorney"
    Then For PCQS scenario Logging in as Internal user and verifying "OCC Attorney"
  @PCQSE2EOCCATT
