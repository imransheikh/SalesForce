Feature: Validate all expected feature is present in Application Receipt and Personal detail page and also validate different search criteria in cutomer search page
  Verify all expected feature is present in Application Receipt and Personal detail page with Call Center ISO user
  @PCQSE2ECCISO
  Scenario: Log in to Saleforce application
    ...
    When For PCQS scenario Search for required Internal User "Call Center ISO"
    Then For PCQS scenario Logging in as Internal user and verifying "Call Center ISO"
  @PCQSE2ECCISO
