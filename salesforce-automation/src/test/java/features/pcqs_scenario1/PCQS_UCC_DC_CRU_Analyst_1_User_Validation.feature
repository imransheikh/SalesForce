Feature: Validate all expected feature is present in Application Receipt and Personal detail page and also validate different search criteria in cutomer search page
  Verify all expected feature is present in Application Receipt and Personal detail page with UCC-DC CRU Analyst 1
  @PCQSE2EUCCDCCRU
  Scenario: Log in to Saleforce application
    ...
    When For PCQS scenario Search for required Internal User "UCC-DC CRU Analyst 1"
    Then For PCQS scenario Logging in as Internal user and verifying "UCC-DC CRU Analyst 1"
  @PCQSE2EUCCDCCRU
  ...
  Scenario: Validate customer page after refresh
  	Then For PCQS scenario of UCC DC CRU To validate post refresh opened tab for "PAULA TURANO"
   @PCQSE2EUCCDCCRU
