Feature: Validate all expected feature is present in Application Receipt and Personal detail page and also validate different search criteria in cutomer search page
  Verify all expected feature is present in Application Receipt and Personal detail page with HD ISO VSC user
  @PCQSE2EHDISOVSC
  Scenario: Log in to Saleforce application
    ...
    When For PCQS scenario Search for required Internal User "HD ISO VSC"
    Then For PCQS scenario Logging in as Internal user and verifying "HD ISO VSC"
  @PCQSE2EHDISOVSC
  ...
  Scenario: Validate customer page after refresh
  	Then For PCQS scenario of HD ISO VSC To validate post refresh opened tab for "PAULA TURANO"
   @PCQSE2EHDISOVSC
