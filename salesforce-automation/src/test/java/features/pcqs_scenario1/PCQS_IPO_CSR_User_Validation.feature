Feature: Validate all expected feature is present in Application Receipt and Personal detail page and also validate different search criteria in cutomer search page
  Verify all expected feature is present in Application Receipt and Personal detail page with IPO CSR user
  @PCQSE2EIPOCSR
  Scenario: Log in to Saleforce application
    ...
    When For PCQS scenario Search for required Internal User "IPO CSR"
    Then For PCQS scenario Logging in as Internal user and verifying "IPO CSR"
  @PCQSE2EIPOCSR
  ...
  Scenario: Validate customer page after refresh
  	Then For PCQS scenario of IPO CSR To validate post refresh opened tab for "PAULA TURANO"
   @PCQSE2EIPOCSR
